...
from dataset.dataset import AVADataset, AVADataset_mean, AVADataset_binary

from model.model import NIMA, Emd_loss
...
def main(config):
    ...
    model = NIMA(base_model, num_classes=1)
    # model = models.VGG(base_model, num_classes=1)
    ...
    # criterion = Emd_loss(r=2).to(device)
    ...
    if config.train:
        ...
        trainset = AVADataset_mean(csv_file=config.train_csv_file, root_dir=config.img_path, transform=train_transform)
        # trainset = AVADataset_binary(csv_file=config.train_csv_file, root_dir=config.img_path, transform=train_transform)
        # valset = AVADataset(csv_file=config.val_csv_file, root_dir=config.img_path, transform=val_transform)
        valset = AVADataset_mean(csv_file=config.val_csv_file, root_dir=config.img_path, transform=val_transform)
        # valset = AVADataset_binary(csv_file=config.val_csv_file, root_dir=config.img_path, transform=val_transform)
        ...
        for epoch in range(config.warm_start_epoch, config.epochs):
            ...
            for i, data in enumerate(train_loader):
                ...
                labels = data['annotations'].to(device).double()
                ...
                print('Epoch: %d/%d | Step: %d/%d | Training EMD loss: %.4f' % (epoch + 1, config.epochs, i + 1, len(trainset) // config.train_batch_size + 1, loss.item()))
            ...
            for data in val_loader:
                images = data['image'].to(device)
                # labels = data['annotations'].to(device).float()
                labels = data['annotations'].to(device)
                with torch.no_grad():
    ...
    if config.test:
        ...
        testset = AVADataset(csv_file=config.test_csv_file, root_dir=config.img_path, transform=test_transform)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--train_csv_file', type=str, default='./dataset/annotations_train_norm.csv')
    parser.add_argument('--val_csv_file', type=str, default='./dataset/annotations_val_norm.csv')
    parser.add_argument('--test_csv_file', type=str, default='./dataset/annotations_test_norm.csv')

    # training parameters
    parser.add_argument('--train', action='store_false')
    parser.add_argument('--test', action='store_true')
    parser.add_argument('--decay', action='store_false')
    ...
    parser.add_argument('--save_fig', action='store_false')
